...
test('User not in proper group; expect redirect', async () => {
  ...
  expect(_event.transaction.redirect_uri).toEqual("https://wiki.mozilla.org/GitHub/SAML_issues?auth=dev&dbg=ghgr");
  expect(fetch).toHaveBeenCalled();
  expect(api.access.deny).toHaveBeenCalled();
...
test('Users github username is undefined; expect redirect', async () => {
  ...
  expect(_event.transaction.redirect_uri).toEqual("https://wiki.mozilla.org/GitHub/SAML_issues?auth=dev&dbg=ghnd");
  expect(fetch).toHaveBeenCalled();
  expect(api.access.deny).toHaveBeenCalled();
...
test('Failed to find users github username; expect redirect', async () => {
  ...
  expect(_event.transaction.redirect_uri).toEqual("https://wiki.mozilla.org/GitHub/SAML_issues?auth=dev&dbg=ghul");
  expect(fetch).toHaveBeenCalled();
  expect(api.access.deny).toHaveBeenCalled();
  ...
  expect(combinedLogs.some(element => element.startsWith(searchString))).toBe(true);
});
