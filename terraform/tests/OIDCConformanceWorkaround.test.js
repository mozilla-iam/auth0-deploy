...
const { onExecutePostLogin } = require('../actions/OIDCConformanceWorkaround.js');
...
describe('Ensure multiple clientID coverage', () => {
  test.each(
    client_ids
  )('given client_id %s, expect event.idToken.updated_at is an int', async (client_id) => {
    ...
    expect(api.idToken.setCustomClaim).toHaveBeenCalledWith("updated_at", 1725218182);
    });
});
