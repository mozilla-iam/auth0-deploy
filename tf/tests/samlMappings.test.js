...
describe("Matrix idToken tests", () => {
  ...
  test.each(clientIDs)(
    "Ensure OIDC claim mapping for client %s",
    async (clientID) => {
      ...
      const idTokenMatches = extractMatchingPairs(
        _idToken,
        expectedIdTokenAttributes
      );
...
describe("Stripe-Subplat SAML tests", () => {
  ...
  const maps = clientIDs.flatMap((clientID) =>
    roles.map((role) => [clientID, role])
  );
  test.each(maps)(
    "Ensure SAML configuration mappings for client %s",
    async (clientID, { role, group }) => {
      _event.client.client_id = clientID;
...
describe("Acoustic SAML tests", () => {
  const clientIDs = [
    "inoLoMyAEOzLX1cZOvubQpcW18pk4O1S",
    "sBImsybtPPLyWlstD0SC35IwnAafE4nB",
  ];
...
describe("Bitsight SAML tests", () => {
  ...
  const bitSightRoles = [
    ...
    {
      role: "Customer Portfolio Manager",
      group: "mozilliansorg_bitsight-users",
    },
    { role: "Customer User", group: undefined },
  ];

  const maps = clientIDs.flatMap((clientID) =>
    bitSightRoles.map((role) => [clientID, role])
  );
...
describe("Google SAML tests", () => {
  const clientIDs = [
    "q0tFB9QyFIKqPOOKvkFnHMj2VwrLjX46",
    "uYFDijsgXulJ040Os6VJLRxf0GG30OmC",
  ];
  const domains = ["mozilla.com", "mozillafoundation.org", "getpocket.com"];

  const maps = clientIDs.flatMap((clientID) =>
    domains.map((domain) => [clientID, domain])
  );

  test.each(maps)(
    "Ensure SAML configuration mappings for client %s with %s",
    async (clientID, domain) => {
      _event.client.client_id = clientID;
    ...
    async (clientID) => {
      ...
      const expectedSamlAttributes = {
        ...
        "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname":
          _event.user.given_name,
        "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname":
          _event.user.family_name,
...
describe("Navex SAML tests", () => {
  const clientIDs = [
    "gL08r5BRiweqf4aDQVX6xB4FHyFepFlM",
    "iz2qSHo0lSv2nRZ8V3JnOESX5UR4dcpX",
  ];
