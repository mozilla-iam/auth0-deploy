...
describe('Test group merges', () => {
  test('Expect user.groups to be a merged list of multiple group lists', async () => {
    ...
    _event.user.app_metadata.groups = ["app_metadata_groups_1", "app_metadata_groups_2"];
    ...
    expect(_idToken["https://sso.mozilla.com/claim/groups"]).toEqual(expect.arrayContaining(mergedGroups));
    expect(mergedGroups).toEqual(expect.arrayContaining(_idToken["https://sso.mozilla.com/claim/groups"]));

    // Additionally, check if they have the same length to ensure no duplicates
    expect(_idToken["https://sso.mozilla.com/claim/groups"]).toHaveLength(mergedGroups.length);
  });
});
...
describe('Client does not exist in apps.yml', () => {
  ...
  test('Client id does not exist and aai is empty; expect not allowed', async () => {
    ...
    expect(_event.transaction.redirect_uri).toBeDefined();
    expect(decodeRedirect(_event.transaction.redirect_uri)).toEqual("aai_failed");
  });
});
 ...
 */
  test('No users, no groups defined; expect not allowed', async () => {
    ...
    expect(_event.transaction.redirect_uri).toBeDefined();
    expect(decodeRedirect(_event.transaction.redirect_uri)).toEqual("notingroup");
